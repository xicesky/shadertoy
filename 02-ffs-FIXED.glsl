...
vec2 sdScene(float time, vec3 p) {
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            ...
            float checkerboard = mod(floor(instance.x) + floor(instance.z), 2.0);
            ...
            float be_funny = checkerboard < 1.0 ? -0.5 : 0.5;
...
vec3 sdfVisualize(float time, vec2 uv) {
    ...
    float t = 2.0 * sin(time * 0.25);  // Example time variation, can be removed if not needed
